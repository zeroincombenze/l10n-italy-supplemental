...
class AssetJournalXslx(models.AbstractModel):
    ...
    def __init__(self, pool, cr):
        ...
        self.depreciation_line_accounting_doc_data = None
    ...
    def set_formats(self):
        ...
        self.format_depreciation_year_line_value_center = self.workbook.add_format(
            {
                "align": "center",
                "font_size": 12,
            }
        )
        self.format_depreciation_year_line_value_right = self.workbook.add_format(
            {
                "align": "right",
                "font_size": 12,
            }
    ...
    def set_asset_accounting_doc_data(self):
        self.asset_accounting_doc_data = self.generate_asset_accounting_doc_data()
    ...
    def set_depreciation_line_year_data(self):
        self.depreciation_line_year_data = self.generate_depreciation_line_year_data()
    ...
    def set_depreciation_line_accounting_doc_data(self):
        self.depreciation_line_accounting_doc_data = (
            self.generate_depreciation_line_accounting_doc_data()
        )

    def generate_depreciation_line_accounting_doc_data(self):
        data = (
            {
                "title": _("Partner"),
                "field": "partner_name",
    ...
    def get_max_width_dict(self):
        min_col = min(
            [
                ...
                min(self.depreciation_line_accounting_doc_data.keys()),
        ...
        max_col = max(
            [
                ...
                max(self.depreciation_line_accounting_doc_data.keys()),
        ...
        return {
            col: max(
                [
                    ...
                    self.depreciation_line_year_data.get(col, {}).get("width") or 0,
                    self.depreciation_line_amount_detail_data.get(col, {}).get("width")
                    or 0,
                    self.depreciation_line_accounting_doc_data.get(col, {}).get("width")
                    or 0,
    ...
    def _write_report_title(self, title):
        ...
        * Overrides standard method *
        ...
        self.sheet.merge_range(
            self.row_pos, 0, self.row_pos, 9, title, self.format_title
        )
        self.row_pos += 3
            ...
            for asset_section in categ_section.report_asset_ids:
                ...
                for dep_section in asset_section.report_depreciation_ids:
                    ...
                    for year in dep_section.report_depreciation_year_line_ids:
                        self.write_value(self.depreciation_line_year_data, year)
                        ...
                        for doc in year.report_accounting_doc_ids:
                            self.write_value(
                                self.depreciation_line_accounting_doc_data, doc
                            )

                if asset_section.report_sale_doc_id:
                    self.write_all(
                        self.asset_accounting_doc_data, asset_section.report_sale_doc_id
                    )
                self.row_pos += 1
