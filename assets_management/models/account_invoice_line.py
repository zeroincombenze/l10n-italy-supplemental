...
class AccountInvoiceLine(models.Model):
    ...
    asset_accounting_info_ids = fields.One2many(
        "asset.accounting.info", "invoice_line_id", string="Assets Accounting Info"
    )
        ...
        for inv_line in self:
            ...
            if len(comp) > 1 or (comp and comp != inv_line.company_id):
                raise ValidationError(
                    _(
                        "`{}`: cannot change invoice line's company once it's"
    ...
    def get_asset_purchase_amount(self, currency=None):
        ...
        for line in self:
            ...
            purchase_amount += line.currency_id.compute(line.price_subtotal, currency)
