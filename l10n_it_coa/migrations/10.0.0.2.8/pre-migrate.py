...
def update_rc_tax_codes(cr):
    ...
    def rm_translation(model, res_id):
        ...
        recs = tnl_model.search(
            [("name", "=like", "%s%%" % model), ("res_id", "=", res_id)]
        )
        if recs:
            recs.unlink()

    def update_tax_records(tax_model):
        chart_template_id = env.ref("l10n_it_coa.l10n_chart_it_zeroincombenze")
        action_done = False
        domain = (
            [("chart_template_id", "=", chart_template_id.id)]
            if hasattr(tax_model, "chart_template_id")
            else []
        )
        ...
        for tax in tax_model.search(domain):
            if tax.description == "a17v" or tax_model.search(
