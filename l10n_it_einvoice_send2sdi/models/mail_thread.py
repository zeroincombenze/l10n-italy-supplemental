...
class MailThread(models.AbstractModel):
    ...
    def message_route(
        ...
        if any(
            ...
            response_attachments = [
                x for x in message_dict["attachments"] if response_regex.match(x.fname)
            ]
            if response_attachments and fatturapa_attachments:
                ...
                _logger.debug(
                    "Routing FatturaPA PEC E-Mail with Message-Id: {}".format(
                ...
                if att:
                    ...
                    if fetchmail_server.e_inv_notify_partner_ids:
                        self.env["mail.mail"].create(
                            {
                                "subject": _("PEC message [%s] not processed")
                                % message.get("Subject"),
                                "body_html": _(
                                    "<p>"
    ...
    def find_attachment_by_subject(self, subject):
        if "CONSEGNA: " in subject:
            ...
            fatturapa_attachment_out = self.env["fatturapa.attachment.out"].search(
    ...
    def create_fatturapa_attachment_in(self, attachment):
        ...
        if attachment.mimetype == "application/zip":
            ...
            if fatturapa_atts:
                ...
                fatturapa_attachment_in.create({"ir_attachment_id": attachment.id})
