...
class DiscountRisePrice(models.Model):
    ...
    invoice_line_id = fields.Many2one(
        "account.invoice.line", "Related Invoice", ondelete="cascade", index=True
    )
    invoice_id = fields.Many2one(
        "account.invoice", "Related Invoice", ondelete="cascade", index=True
    )
...
class AccountInvoice(models.Model):
    ...
    def einvoice_type_selection(self, type, partner_vat, amount=None):
        ...
        res = [
            x.id
            for x in einv_type_model.search([("scope", "like", scope)], order="code")
        ]
