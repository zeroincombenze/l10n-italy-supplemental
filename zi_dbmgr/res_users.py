...
class res_users(osv.Model):
    ...
    def _auth_dbmgr_signin(self, cr, uid, provider, validation, params,
                           context=None):
        # overridden to use signup method if regular oauth signin fails
        try:
            login = super(res_users, self)._auth_dbmgr_signin(
            ...
            try:
                _, login, _ = self.signup(
                    cr, uid, values, token, context=context)
            except SignupError:
