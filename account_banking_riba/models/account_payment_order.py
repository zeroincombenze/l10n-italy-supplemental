...
class AccountPaymentOrder(models.Model):
    ...
    def generate_payment_file(self):
        ...
        if self.payment_method_id.code != "riba_cbi":
            ...
            self.validate_iban(
                creditor_bank_account.acc_number, "Codice IBAN creditore non valido"
            )
        ...
        if self.payment_method_id.code == "riba_cbi":
            ...
            if self.journal_id.is_wallet is False:
                for line in self.payment_line_ids:
                    line.move_line_id.write({"incasso_effettuato": True})
        else:
            res = super().generated2uploaded()
        return res
    ...
    def _registra_accredito_riba(self):
        ...
        for payment_order in self:
            ...
            if not cfg["liquidity_account_id"]:
                raise UserError("Attenzione!\nConto liquidità bancaria non impostato.")
            if not cfg["accreditation_account_debit_id"]:
                raise UserError("Attenzione!\nConto dare di accredito non impostato.")
            if not cfg["accreditation_account_credit_id"]:
                raise UserError("Attenzione!\nConto avere di accredito non impostato.")
            if not cfg["bank_journal"]:
                raise UserError("Attenzione!\nConto di costo non impostato.")
            if (
                cfg["accreditation2_account_credit_id"]
                and not cfg["accreditation2_account_debit_id"]
            ):
                raise UserError("Attenzione!\nConto dare 2 di accredito non impostato.")
            if (
                cfg["accreditation2_account_debit_id"]
                and not cfg["accreditation2_account_credit_id"]
            ):
                raise UserError(
                    "Attenzione!\nConto avere 2 di accredito non impostato.")

            accredito_lines = [
                *payment_order.prepare_move_lines(
                    cfg["accreditation_account_debit_id"], "debit", mode="auto"
                ),
                *payment_order.prepare_move_lines(
                    cfg["accreditation_account_credit_id"], "credit", mode="auto"
                ),
            ]
            if cfg["accreditation2_account_debit_id"]:
                accredito2_lines = [
                    *payment_order.prepare_move_lines(
                        cfg["accreditation2_account_debit_id"], "debit", mode="auto"
                    ),
                    *payment_order.prepare_move_lines(
                        cfg["accreditation2_account_credit_id"], "credit", mode="auto"
                    ),
                ]
                accredito_lines += accredito2_lines
            ...
            if amount_expense > 0:
                ...
                bank_expense_line = self.payment_line_ids[0].prepare_1_move_line(
                    cfg["liquidity_account_id"], "credit", amount=amount_expense
                )
