...
class AccountInvoice(models.Model):
    ...
    def get_number_from_alpha(self):
        if self.state == 'draft':
            raise UserError('Conversione da alfanumerico numero ad intero '
                            'non permessa se la fattura si trova in bozza.')
        ...
        if self.journal_id:
            ...
            if interpolate_postf in self.number:
                value_number = value_number.replace(interpolate_postf, '')
            # end if

            if interpolate_pref in self.number:
