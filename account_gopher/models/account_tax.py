    ...
    def gopher_configure_tax(self, html_txt=None):
        ...
        for tax in tax_model.search([]):
            ...
            for name in ("account_id", "refund_account_id"):
                if (
                    getattr(tax, name)
                    and getattr(tax, name).company_id.id != cur_company_id
                ):
        ...
        for tax in parsed:
            ...
            if res[tax].get("wgt") or res[tax].get("amt"):
                ...
                if hasattr(tax, "vsc_exclude_operation") and hasattr(
                    tax, "vsc_exclude_vat"
                ):
                ...
                if "pay" in res[tax]:
                    ...
                    if vals.get("payability", False) != tax.payability:
                        if vals.get("payability"):
                            actioned += _("set payability to %s; ") % res[tax].get(
                                "pay"
                            )
                        else:
                ...
                if vals.get("assosoftware_id", False) != tax.assosoftware_id.id:
                    if vals.get("assosoftware_id"):
                        actioned += _("set ax code to %s; ") % res[tax].get("axc")
                    else:
                    ...
                    if tax.rc_type == "self":
                        if not tax.rc_sale_tax_id:
                            ...
                            if tax.amount != tax_sale.amount:
                                ...
                                actioned += _(
                                    "Invalid account company for sale tax %s"
                                    % tax_sale.description
                                )
                            elif getattr(tax_sale, NATURE_ID):
                                actioned = _(
                                    "Invalid NATURE for sale tax %s"
                                    % tax_sale.description
                                )
    ...
    def gopher_reload_taxes(self, html_txt=None):
        ...
        def get_tmpl_values(tmpl, rec=None):
            ...
            for name in ("tax_group_id",):
                if not rec or (
                    getattr(tmpl, name) and getattr(rec, name) != getattr(tmpl, name)
                ):
            ...
            for name in (
                ...
                #     (not rec or not getattr(rec, name))
                #     or (getattr(rec, name).code != getattr(tmpl, name).code)
                # ):
                ...
                if len(acc) == 1 and (not rec or getattr(rec, name).id != acc[0].id):
                    vals[name] = acc[0].id
            return vals
        ...
        for tmpl in template_model.search(
            [("chart_template_id", "=", chart_template_id.id)], order="sequence"
        ):
