...
class AccountBankStatementLine(models.Models):
    ...
    def get_move_lines_for_reconciliation(
        ...
        reconciliation_aml_accounts = [self.journal_id.default_credit_account_id.id,
        ...
        if self.partner_id.id or overlook_partner:
            domain_matching = expression.AND(
                [domain_matching, [('account_id.internal_type', 'in', ['payable', 'receivable'])]])
        else:
        ...
        generic_domain = self.env['account.move.line'].with_context(ctx).domain_move_lines_for_reconciliation(
