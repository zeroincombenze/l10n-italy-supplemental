...
# if user.login[1]=='0':        # [antoniov 2014-07-14 chanded with
# SUPERUSER_ID
        if uid != SUPERUSER_ID:
            ...
            if data_da_controllare:
                ...
                if year_invoice > 2013:                        # Before 2014, records are permitted
                    ...
                    user_acc_ids = user_acc_obj.search(
                        cr, uid, [('name', '=', user_login), ('year', '=', year_invoice)])
                    if user_acc_ids:
                        cli = user_acc_obj.browse(cr, uid, user_acc_ids)[0]
                        if cli.period_0 == False:
                            if month_invoice == '01' and cli.period_1 == False:
                                raise orm.except_orm(_('Errore %s') % nome_data, _(
                                    'Il tuo utente non è abilitato a registrare in Gennaio %s.') % year_invoice)
                            if month_invoice == '02' and cli.period_2 == False:
                                raise orm.except_orm(_('Errore %s') % nome_data, _(
                                    'Il tuo utente non è abilitato a registrare in Febbraio %s.') % year_invoice)
                            if month_invoice == '03' and cli.period_3 == False:
                                raise orm.except_orm(_('Errore %s') % nome_data, _(
                                    'Il tuo utente non è abilitato a registrare in Marzo %s.') % year_invoice)
                            if month_invoice == '04' and cli.period_4 == False:
                                raise orm.except_orm(_('Errore %s') % nome_data, _(
                                    'Il tuo utente non è abilitato a registrare in Aprile %s.') % year_invoice)
                            if month_invoice == '05' and cli.period_5 == False:
                                raise orm.except_orm(_('Errore %s') % nome_data, _(
                                    'Il tuo utente non è abilitato a registrare in Maggio %s.') % year_invoice)
                            if month_invoice == '06' and cli.period_6 == False:
                                raise orm.except_orm(_('Errore %s') % nome_data, _(
                                    'Il tuo utente non è abilitato a registrare in Giugno %s.') % year_invoice)
                            if month_invoice == '07' and cli.period_7 == False:
                                raise orm.except_orm(_('Errore %s') % nome_data, _(
                                    'Il tuo utente non è abilitato a registrare in Luglio %s.') % year_invoice)
                            if month_invoice == '08' and cli.period_7 == False:
                                raise orm.except_orm(_('Errore %s') % nome_data, _(
                                    'Il tuo utente non è abilitato a registrare in Agosto %s.') % year_invoice)
                            if month_invoice == '09' and cli.period_7 == False:
                                raise orm.except_orm(_('Errore %s') % nome_data, _(
                                    'Il tuo utente non è abilitato a registrare in Settembre %s.') % year_invoice)
                            if month_invoice == '10' and cli.period_10 == False:
                                raise orm.except_orm(_('Errore %s') % nome_data, _(
                                    'Il tuo utente non è abilitato a registrare in Ottobre %s.') % year_invoice)
                            if month_invoice == '11' and cli.period_11 == False:
                                raise orm.except_orm(_('Errore %s') % nome_data, _(
                                    'Il tuo utente non è abilitato a registrare in Novembre %s.') % year_invoice)
                            if month_invoice == '12' and cli.period_12 == False:
                                raise orm.except_orm(_('Errore %s') % nome_data, _(
                                    'Il tuo utente non è abilitato a registrare in Dicembre %s.') % year_invoice)
                    else:
                        raise orm.except_orm(_('Errore %s') % nome_data, _(
                            'Il tuo utente non è abilitato a registrare!'))
        return True
