...
class StockPickingPackagePreparation(models.Model):
    ...
    def get_max_package(self, packaging_ids=None):
        ...
        by_dimension = (
            by_length
        ) = by_width = by_height = by_volume = by_weight = packType
        ...
        if packaging_ids and (len(packaging_ids) > 1):
            ...
            by_volume = sorted(packaging_ids, key=attrgetter('max_volume'))[
                last
            ]
            by_weight = sorted(packaging_ids, key=attrgetter('max_weight'))[
                last
            ]
            by_dimension, dimension = get_dimension(
                by_length, by_width, by_height
            )
        return {
            'length': by_length,
            'width': by_width,
        ...
        if packaging_ids and move_line:
            ...
            if move_id:
                ...
                if max_pack['weight'].max_weight >= max_weight:
                    by_volume = packType
                    by_weight = sorted(
                        packaging_ids, key=attrgetter('max_weight')
                    )
                    for packaging_id in by_weight:
    ...
    def get_max_products(self, move_lines=None):
        ...
        by_dimension = (
            by_length
        ) = by_width = by_height = by_volume = by_weight = productType
    ...
    def pack_order_lines_00(self, move_lines, packaging_ids):
        ...
        if move_lines:
            ...
            for move_line in move_lines:
                if move_line:
                    if not packaging_id:
                        packaging_id = self.get_package(
                            packaging_ids, remaining_lines
                        )
                        volume_limit = packaging_id.max_volume or 0.0
                        weight_limit = packaging_id.max_weight or 0.0
                    qty_line = 0
                    ...
                    for item_quant in range(residual):
                        ...
                        if packaging_id:
                            if ((weight + this_weight) < weight_limit) and (
                                (volume + this_volume) < volume_limit
                            ):
                                weight += this_weight
                                volume += this_volume
            ...
            if pre_packing_lines:
                ...
                item_name = "{}".format(iteration)
                packed_lines.update({item_name: [pre_packing_lines, values]})
        ...
        if move_lines:
            ...
            for move_line in move_lines:
                if move_line:
                    ...
                    if not packaging_id:
                        packaging_id = self.get_package_by_line(
                            packaging_ids, move_line
                        )
                    if not packaging_id:
                        packaging_id = self.get_max_right_package(
                            packaging_ids, remaining_lines
                        )
                    ...
                    for item_quant in range(residual):
                        ...
                        line_value = {
                            ...
                            "reserved_quantity": qty_line + quantity
                            if (reserved > 0)
                            else 0,
                        ...
                        if packaging_id:
                            ...
                            if check_weight and check_volume:
                                ...
                                if packaging_id:
                                    ...
                                    volume_limit = (
                                        packaging_id.max_volume or 0.0
                                    )
                                    weight_limit = (
                                        packaging_id.max_weight or 0.0
                                    )
                        else:
                            weight += this_weight
                            volume += this_volume
    ...
    def _generate_pack(self):
        ...
        if not move_lines:
            raise UserError(_('No transfers to perform.'))

        packaging_ids = self.packaging_ids
        if not packaging_ids:
            packaging_ids = packaging_model.search(
                [('product_id', '=', False)]
            )
        # TODO: Manage packagings by the whole set rather dedicated one
        if not packaging_ids:
            raise UserError(_('No packagings available.'))
        ...
        if packed_lines:
            ...
            for item_package in packed_lines.keys():
                ...
                for item_in_package in pre_packing_lines.keys():
                    ...
                    quant_line_model._reset_reserved_quantity(
                    ...
                    quant_line_model._update_reserved_quantity(
                        move_line.product_id,
                        move_line.location_id,
                        (line_qty + item_qty),
                        lot_id=move_line.lot_id,
                ...
                packed_lines.update({item_package: [packing_lines, values]})
