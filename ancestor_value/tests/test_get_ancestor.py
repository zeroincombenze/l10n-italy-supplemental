...
class TestInvoiceRound(SingleTransactionCase):
    ...
    def get_text(self, html):
        # From Odoo 16.0, comment field became html, so we have to extract text
        # in order to validate tests
        items = re.split("<[a-z/]+>", html)
        return items[0] if len(items) == 1 else items[1]
    ...
    def test_ancestor_value(self):
        ...
        self.assertEqual(self.get_text(record_middle.comment), "middle")
        self.assertEqual(self.get_text(
            record_middle.ancestor_value("ref", field="comment")), "top")
        ...
        self.assertEqual(self.get_text(record_bottom.comment), "bottom")
        self.assertEqual(self.get_text(
            record_bottom.ancestor_value("comment")), "bottom")
        self.assertEqual(self.get_text(
            record_bottom.ancestor_value("comment", skip_current=True)), "middle")
        self.assertEqual(self.get_text(
            record_bottom.ancestor_value("ref", field="comment")), "top")
