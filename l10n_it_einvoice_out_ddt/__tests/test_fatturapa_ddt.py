...
class TestInvoiceDDT(FatturaPACommon):
    def setUp(self):
        ...
        self.carriage_condition_PF = self.env.ref("l10n_it_ddt.carriage_condition_PF")
        self.goods_description_CAR = self.env.ref("l10n_it_ddt.goods_description_CAR")
    ...
    def test_e_invoice_ddt(self):
        ...
        invoice_wizard = (
            ...
            .with_context({"active_ids": (self.so1.ddt_ids | self.so2.ddt_ids).ids})
            .create({})
        )
        ...
        invoice_wizard = (
            self.env["ddt.create.invoice"]
            .with_context({"active_ids": self.so3.ddt_ids.ids})
            .create({})
        )
        action = invoice_wizard.create_invoice()
        invoice_ids = action["domain"][0][2]
        ...
        xml_content = attachment.datas.decode("base64")
        self.assertEqual(attachment.datas_fname, "IT06363391001_00007.xml")
        self.check_content(
            xml_content,
            "IT06363391001_00007.xml",
