...
class WizardAccountInvoiceImportZip(models.TransientModel):
    ...
    type = fields.Selection(
        [("purchase", "Purchase Invoices"), ("sale", "Sale Invoices")],
        "Invoice type",
        default="purchase",
    )
    ...
    def import_zip(self):
        ...
        rex = r"[A-Z]{2}[A-Za-z0-9]+_[A-Za-z0-9]{3,5}\.(xml|XML|xml.p7m|XML.P7m)"
        ...
        for xml_fullfile in zf.namelist():
            ...
            if re.match(rex, xml_file):
                ...
                try:
                    ...
                    raise UserError(
                        "Error %s extracting %s from zip file" % (e, xml_fullfile)
                    )
        return {
