...
class SaleOrder(models.Model):
    ...
    def copy(self, default=None):
        ...
        if self.env.context.get('new_sale_revision'):
            ...
            saved_self = self
            while self and not self.active:
                new_rev_number = self.revision_number + 1
                new_self = self.search([(
                    "name",
                    "=",
                    "%s-%02d" % (self.unrevisioned_name, new_rev_number))])
                self = new_self or self
                if not new_self:
                    break
            new_rev_number = self.revision_number + 1
            default.update({
            ...
            new_revision = super(SaleOrder, saved_self).copy(default)
