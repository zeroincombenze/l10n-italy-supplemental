# Copyright 2021-2023 LibrERP enterprise network <https://www.librerp.it>
#
...
OPPOSITE_NATURE = {"A": "P", "P": "A", "C": "R", "R": "C"}
LINE_SEL = {
    "trial": "1",
    "ordinary": "2",
}
AMOUNT_NAMES = ("amount_start", "amount_debit", "amount_credit", "amount_balance")
...
ACCRUAL_MESSAGES = {
    ...
    "invtobericeived_passive": {
        ...
        "wrong_type": "Conto Fatture fornitori da ricevere di tipo " "non corretto.",
        "nature": "P",
    },
...
class WizardGenerateBalance(models.TransientModel):
    ...
    def show_profile_warning(self):
        ...
        warning_mess = {"title": "Profile contabile non rilevato", "message": message}
    ...
    date_from = fields.Date(
        string="Start date", default=_set_date_from, readonly=True, required=True
    )

    date_to = fields.Date(
        string="End date", default=_set_date_to, readonly=True, required=True
    )
    ...
    order_by = fields.Selection(
        [
            ("APCRO", "Patrimoniale,Costi,Ricavi,Ordine"),
    ...
    select_details = fields.Selection(
        [
            ...
            ("customers_suppliers", "Clienti e fornitori"),
    ...
    receivable_account_only = fields.Boolean(
        "Receivable account only", readonly=True, default=_set_receivable_account_only
    )
    payable_account_only = fields.Boolean(
        "Payable account only", readonly=True, default=_set_payable_account_only
    )
    no_grand_total = fields.Boolean(
        "No grand total", default=_set_no_grand_total, readonly=True
    )
    no_nature_total = fields.Boolean(
        "No nature total", readonly=True, default=_set_no_nature_total
    )
    no_selected_moves = fields.Boolean(
        "No selected moves", default=_set_no_selected_moves, readonly=True
    )
    inversion = fields.Selection(
        [
            ("no_inversion", "Non invertire"),
            ("account_inversion", "Inverti in base al " "corrispondente del conto"),
    ...
    last_update = fields.Datetime(
        string="Ultimo aggiornamento", default=_set_last_update, readonly=True
    )
    ...
    def lpad_name_by_level(self, name, level_balance):
        return "%s%s" % (INDENTING_SPACE * int(level_balance), name.strip())

    def vals_from_sqlrow(self, row, params, nature=None, seq2=None, level_balance=None):
        ...
        if level_balance == LEVEL_BALANCE_PARTNER and len(row) > 10 and row[PNAME]:
            vals["name"] = self.lpad_name_by_level(row[PNAME], level_balance)
            vals["pos_balance"] = "S"
        return vals, nature

    def amounts_from_sqlrow(
        self, row, vals, reset_mode=None, evaluate_mode=None, add_mode=None
    ):
        if reset_mode:
        ...
        if evaluate_mode:
            vals["amount_balance"] = row[AMT_START] + row[AMT_DBT] - row[AMT_CRD]
        return vals
    ...
    def vals_from_rec(self, rec, nature, params, seq1=None):
        ...
        for nm in (
            ...
            vals[nm] = rec[nm]
            if nm == "name":
                vals[nm] = self.lpad_name_by_level(rec[nm], rec["level_balance"])
        for nm in ("parent_code", "group_id", "account_id", "partner_id", "parent_id"):
            if nm in rec:
                if nm.endswith("_id") and not isinstance(rec[nm], int):
                    vals[nm] = rec[nm].id
                else:
                    vals[nm] = rec[nm]
        return vals
    ...
    def add_values_to_rec(self, rec, vals):
        if self.balance_type == "trial":
            ...
            vals["amount_balance"] = (
                vals["amount_start"] + vals["amount_debit"] - vals["amount_credit"]
            )
        else:
        ...
        if vals.get("account_id"):
            ...
            if account.opposite_sign_code:
                ...
                vals["name"] = self.lpad_name_by_level(
                    account.opposite_sign_code.name, vals["level_balance"]
                )
            elif account.opposite_sign_description:
                vals["name"] = self.lpad_name_by_level(
                    account.opposite_sign_description, vals["level_balance"]
                )
        return vals
    ...
    def write_1_line(
            ...
            if (nature in ("A", "C") and vals["amount_balance"] < 0.0) or (
                ...
                if vals["account_id"]:
                    ...
                    if self.inversion == "force_inversion" or (
                        self.inversion == "account_inversion"
                        and account.negative_balance in ("invert_on_demand", "invert")
                        or account.negative_balance == "invert"
                    ):
        ...
        domain = [("code", "=", vals["code"]), ("balance_id", "=", vals["balance_id"])]
    ...
    def write_lines_from_query(self, line_query, params):
        ...
        def init_vals_partner(row, seq, params):
            vals, nature = self.vals_from_sqlrow(
                row, params, seq2=seq, level_balance=LEVEL_BALANCE_PARTNER
            )
    ...
    def evaluate_parent_lines(self, params):
        ...
        def sum_nature(parents, nature, row):
            ...
            for ii, nm in enumerate(
                ...
                setattr(self, nm, getattr(self, nm) + (row[ii + AMT_START] or 0.0))
            return parents

        def sum_amounts(parents, parent_id, row, nature=None, sub=None, level=None):
        ...
        for nature in self.order_by:
            ...
            for parent_id in parents.keys():
                vals = self.vals_from_rec(parents[parent_id], nature, params)
                if vals.get("account_id"):
                    account = self.env["account.account"].browse(vals["account_id"])
                    if (
                        account.nature != nature
                        and self.inversion == "force_inversion"
                        or (
                            self.inversion == "account_inversion"
                            and account.negative_balance
                            in ("invert_on_demand", "invert")
                        )
                    ):
    ...
    def write_L_n_P(self, params, sel=None):
        ...
        Supplemental line like L&P, "Earning" etc must have a code as follows:
        ...
        # prior year loss/profit
        base_vals.update({"code": "^", "seq2": SEQ2_LAST - 4})
        if self.prior_earning > 0:
            vals = base_vals.copy()
            vals["name"] = "* PERDITA DA ESERCIZIO PRECEDENTE *"
            ...
            vals = base_vals.copy()
            vals["name"] = "* UTILE DA ESERCIZIO PRECEDENTE *"
        ...
        # current year loss/profit
        base_vals.update({"code": "-", "seq2": SEQ2_LAST - 3})
        if self.loss_profit_a > 0:
            vals = base_vals.copy()
            vals["name"] = "* PERDITA LORDA DA RILEVARE *"
            ...
            vals = base_vals.copy()
            vals["name"] = "* UTILE LORDO PREIMPOSTE *"
        ...
        if self.loss_profit < 0:
            vals = base_vals.copy()
            vals["name"] = "* PERDITA DI ESERCIZIO *"

            if (self.balance_type in ("ordinary", "opposite") and nature_LP == "R") or (
                self.balance_type not in ("ordinary", "opposite") and nature_LP == "C"
            ):
            ...
            vals = base_vals.copy()
            vals["name"] = "* PROFITTI ANTE IMPOSTE *"
            if (self.balance_type in ("ordinary", "opposite") and nature_LP == "R") or (
                self.balance_type not in ("ordinary", "opposite") and nature_LP == "C"
            ):
                vals["amount_balance"] = -self.loss_profit
            ...
            vals = base_vals.copy()
            vals.update({"name": "PROFITTI & PERDITE", "amount_balance": 0.0})
        self.write_1_line(vals, sel=sel)
        ...
        for cur_nature in nature_list:
            ...
            for cur_line in self.line_model[cur_nature].search(domain):
                ...
                if cur_nature in ["CD", "SD"]:
                    ...
                    cur_domain.append(("level_balance", "!=", LEVEL_BALANCE_PARTNER))

                if (
                    cur_nature in ["CD", "SD"]
                    and self.view_partner_details != "in_balance"
                ):
                    continue
                ...
                if cur_nature in ["CD", "SD"]:
                    line_nature = (
                        ...
                        .search([("code", "=", cur_line.code)])
                        .nature
                        or ""
                    )
                    vals = self.vals_from_rec(
                        cur_line, line_nature, params, seq1=cur_line["seq1"]
                    )
                else:
                    vals = self.vals_from_rec(
                        cur_line, cur_nature, params, seq1=cur_line["seq1"]
                    )
    ...
    def init_lines(self, params):
        ...
        for nature in nature_list:
            query = self.build_sql_block(
                {"1": self.line_table[nature]}, params, initial="UPDATE "
            )
            if nature == "2":
                query += self.build_sql_block(
                    {
            ...
            query += self.build_sql_block(
                {"1": "balance_id=%(active_id)s"}, params, initial=" WHERE "
            )
            self.env.cr.execute(query)
            # Delete dirty records
            if nature in ID_2_NATURE:
                query = self.build_sql_block(
                    {"1": self.line_table[nature]}, params, initial="DELETE FROM "
                )
                query += self.build_sql_block(
                    {
    ...
    def init_lines_accrual(self, params):
        ...
        query = self.build_sql_block(
            {"1": "italy_account_balance_line_accrual"}, params, initial="UPDATE "
        )
        query += self.build_sql_block(
            {
                "1": "to_delete=true",
    ...
    def purge_lines_accrual(self, params):
        query = self.build_sql_block(
            {"1": "italy_account_balance_line_accrual"}, params, initial="DELETE FROM "
        )
        query += self.build_sql_block(
            {"1": "balance_id=%(active_id)s", "2": "to_delete=true"},
            params,
    ...
    def purge_by_line_level(self, params):
        ...
        if self.hierarchy_level and self.hierarchy_level != "0":
            ...
            for nature in selection:
                ...
                query += self.build_sql_block(
                    where_and_conditions, params, sep=" AND ", initial=" WHERE "
                )
                self.env.cr.execute(query)
    ...
    def purge_partner_balance(self, params):
        ...
        query = self.build_sql_block(
            {"1": balance_line_name}, params, initial="DELETE FROM "
        )
        query += self.build_sql_block(
            {
                "1": "balance_id=%(active_id)s",
    ...
    def build_main_query(self, params):
        ...
        if not self.no_opening_balances:
            ...
            sql_what = {
                ...
                "08": "c.amount_credit-" "c.amount_credit AS amount_balance",
            }
        if self.select_details in (
            "customers_suppliers",
            "only_customers",
        ...
        if self.date_balance_option == "competence_date":
            sql_domain_cur = {
                "1":
                    "COALESCE(%(m.date_apply_balance)s, %(m.date)s) >= '%(date_from)s'",
        ...
        if (
            self.nature_of_accounts
            and self.nature_of_accounts in nature_transcoding.keys()
        ):
        ...
        # Query previous year
        prior_query = self.build_sql_block(
            sql_what_prior, params, sep=",", initial="SELECT "
        )
        prior_query += self.build_sql_block(sql_from, params, sep=",", initial=" FROM ")
        prior_query += self.build_sql_block(
            sql_domain, params, sep=" AND ", initial=" WHERE "
        )
        prior_query += self.build_sql_block(
            sql_domain_prior, params, sep=" AND ", initial=" AND "
        )
        prior_query += self.build_sql_block(
            sql_group_prior, params, sep=",", initial=" GROUP BY "
        )
        # Query current year
        cur_query = self.build_sql_block(
            sql_what_cur, params, sep=",", initial="SELECT "
        )
        cur_query += self.build_sql_block(sql_from, params, sep=",", initial=" FROM ")
        cur_query += self.build_sql_block(
            sql_domain, params, sep=" AND ", initial=" WHERE "
        )
        cur_query += self.build_sql_block(
            sql_domain_cur, params, sep=" AND ", initial=" AND "
        )
        cur_query += self.build_sql_block(
            sql_group_cur, params, sep=",", initial=" GROUP BY "
        )
        ...
        query += self.build_sql_block(sql_what, params, sep=",", initial="SELECT ")
        if not self.no_opening_balances:
        ...
        query += self.build_sql_block(sql_group, params, sep=",", initial=" GROUP BY ")
        ...
        query += self.build_sql_block(sql_order, params, sep=",", initial=" ORDER BY ")
    ...
    def write_accrual(self, params):
        ...
        def build_accrual_query(params):
            ...
            accrual_query = self.build_sql_block(
        ...
        def set_accrual_date(accr_env, row, params):
            def get_date_from(row, params, sel_fy):
                ...
                if (
                    row[DATE_FROM] < params[sel_fy]["date_from"] and sel_fy == "cur_fy"
                ) or sel_fy == "prior_fy":
                    # anno precedente (rateo)
                    out_fy = "<"
                    accr_date_from = max(
                        params["prior_fy"]["date_from"], row[DATE_FROM]
                    )
                elif (
                    row[DATE_FROM] > params[sel_fy]["date_to"] and sel_fy == "cur_fy"
                ) or sel_fy == "next_fy":
                    # anno successivo (risconto)
                    out_fy = ">"
                    accr_date_from = min(params["next_fy"]["date_from"], row[DATE_FROM])
            ...
            def get_date_to(row, params, sel_fy):
                ...
                if (
                    row[DATE_TO] > params[sel_fy]["date_to"] and sel_fy == "cur_fy"
                ) or sel_fy == "next_fy":
                    # anno successivo (risconto)
                    out_fy = ">"
                    accr_date_to = min(params["next_fy"]["date_to"], row[DATE_TO])
                elif row[DATE_TO] < params[sel_fy]["date_from"] and sel_fy == "cur_fy":
                    # anno precedente (rateo)
                    out_fy = "<"
                    accr_date_to = max(params["prior_fy"]["date_to"], row[DATE_TO])
            ...
            accr_env.days_cur = (
                min(accr_date_to, self.date_to) - max(accr_date_from, self.date_from)
            ).days + 1
            ...
            if from_fy == "<" and to_fy == ">":
                ...
                if row[0] == "C":
                    ...
                    accr_env.accrual_type = "cutoff_passive"
                accr_env.days_expired = (accr_date_from - row[DATE_FROM]).days
                accr_env.days_prior = accr_env.days_accrual - accr_env.days_expired
                accr_env.days_cur = 0
            elif from_fy == "<":
                # DATE_FROM < fiscal_year / DATE_TO
                if row[0] == "C":
                ...
                accr_env.days_expired = (accr_date_from - row[DATE_FROM]).days
                accr_env.days_prior = (
                    accr_env.days_accrual - accr_env.days_cur - accr_env.days_expired
                )
            elif from_fy == ">":
        ...
        def set_accrual_values(accr_env, row):
            ...
            accr_env.amount_next = round(
                ...
                - (
                    ...
                    + accr_env.amount_expired
                ),
                precision,
            )
            if (
                accrual_type in ("prepayment_passive", "cutoff_active")
                or not accrual_type
                and row[NATURE] == "R"
            ):
            ...
            if accr_env.days_cur != accr_env.days_accrual:
                accr_env.rated_debit = round(
                    invoice_debit
                    - invoice_debit * accr_env.days_cur / accr_env.days_accrual,
                    precision,
                )
                accr_env.rated_credit = round(
                    invoice_credit
                    - invoice_credit * accr_env.days_cur / accr_env.days_accrual,
                    precision,
                )
        ...
        def set_line_values(vals, accr_env):
            ...
            vals["amount_balance"] = accr_env.rated_debit - accr_env.rated_credit
            return vals
        ...
        def create_accr_env():
            ...
            Accrual = type(
                "Accrual class",
                (),
                {"__setitem__": cls_setitem, "__getitem__": cls_getitem},
            )
            ...
            for accrual_type in (
                ...
                if profile_account and hasattr(profile_account[accrual_type], "code"):
                    accr_env[accrual_type]["account_id"] = (
                        profile_account[accrual_type].id
                    )
                    for nm in ("parent_id", "code", "name"):
                        if nm == "parent_id":
                            mdel = profile_account[accrual_type].group_id
        ...
        for row in self.env.cr.fetchall():
            ...
            accr_type = (
                accr_env.accrual_type
                if not accr_env.accrual_type2
                else "%s+%s" % (accr_env.accrual_type, accr_env.accrual_type2)
            )
            ...
            line = self.line_model[vals["nature"]].search(
                [("code", "=", vals["code"]), ("balance_id", "=", params["active_id"])]
            )
            if line:
            ...
            for accrual_type in (
                ...
                if (
                    accrual_type != accr_env.accrual_type
                    and accrual_type != accr_env.accrual_type2
                ):
                    continue
                ...
                if accrual_type == "cutoff_passive":
                    ...
                    vals["amount_balance"] = accr_env.amount_next
                vals["name"] = self.lpad_name_by_level(
                    accr_env[accrual_type]["name"], level_balance
                )
    ...
    def apply_for_inversion(self, params):
        ...
        for cur_nature in self.order_by:
            for cur_line in self.line_model[cur_nature].search(
                [("balance_id", "=", params["active_id"]), ("nature", "!=", cur_nature)]
            ):
                domain = [
                    ("balance_id", "=", params["active_id"]),
                    ("level_balance", "=", LEVEL_BALANCE_LINE),
                ...
                line = self.line_model[cur_line.nature].search(
                    domain + [("code", ">", cur_line.code)], limit=1, order="seq2"
                )
                max_seq = line[0].seq2 if len(line) else SEQ2_LAST
                line = self.line_model[cur_line.nature].search(
                    domain + [("code", "<", cur_line.code)], limit=1, order="seq2 desc"
                )
                min_seq = line[0].seq2 if len(line) else 10
                vals = self.vals_from_rec(cur_line, cur_line.nature, params)
    ...
    def build_partner_balance_query(self, params):
        ...
        sql_left_join = {"1": "res_partner r", "2": "r.id = l.partner_id"}
        account_type = {
        ...
        if (
            ...
            sql_domain_prior.update(
                {"10": "t.nature IN %s" % (nature_transcoding[self.nature_of_accounts])}
            )
            sql_domain_cur.update(
                {"10": "t.nature IN %s" % (nature_transcoding[self.nature_of_accounts])}
            )
        ...
        sql_from = {"1": "customers_suppliers"}
        sql_group = {
            "1": "nature",
            "2": "code",
            "3": "name",
        ...
        sql_prior = self.build_sql_block(
            sql_what_prior, params, sep=",", initial="SELECT "
        )
        sql_prior += self.build_sql_block(
            sql_from_prior_current, params, sep=",", initial=" FROM "
        )
        sql_prior += self.build_sql_block(
            sql_left_join, params, sep=" ON ", initial=" LEFT JOIN "
        )
        sql_prior += self.build_sql_block(
            sql_domain, params, sep=" AND ", initial=" WHERE "
        )
        ...
        sql_prior += self.build_sql_block(
            sql_group_prior_current, params, sep=",", initial=" GROUP BY "
        )
        sql_current = self.build_sql_block(
            sql_what_cur, params, sep=",", initial="SELECT "
        )
        sql_current += self.build_sql_block(
            sql_from_prior_current, params, sep=",", initial=" FROM "
        ...
        query += ")"
        query += self.build_sql_block(sql_what, params, sep=",", initial="SELECT ")
        query += self.build_sql_block(sql_from, params, sep=",", initial=" FROM ")
        query += self.build_sql_block(sql_group, params, sep=",", initial=" GROUP BY ")
        return query
    ...
    def generate_balance(self):
        ...
        def init_env():
            ...
            for nat in ID_2_NATURE.keys():
                self.line_model_name[nat] = (
                    "italy.account.balance.line.%s" % ID_2_NATURE[nat]
                )
                self.line_model[nat] = self.env[self.line_model_name[nat]]
                self.line_table[nat] = self.line_model_name[nat].replace(".", "_")
            ...
            self.line_model_name["CD"] = "italy.account.balance.line.customer.detail"
            self.line_model["CD"] = self.env[self.line_model_name["CD"]]
            self.line_table["CD"] = self.line_model_name["CD"].replace(".", "_")
            self.line_model_name["SD"] = "italy.account.balance.line.supplier.detail"
            self.line_model["SD"] = self.env[self.line_model_name["SD"]]
            self.line_table["SD"] = self.line_model_name["SD"].replace(".", "_")
            self.seq1 = {}
            self.seq2 = {}
            ...
            if "cur_fy" not in params:
                raise UserWarning("Nessun anno fiscale per il periodo richiesto")
            for fy in params["fys"].values():
                if fy.date_to < self.date_from:
                    if (
                        "prior_fy" not in params
                        or fy.date_to > params["prior_fy"]["date_to"]
                    ):
                        params["prior_fy"] = {
                            "date_from": fy.date_from,
                            "date_to": fy.date_to,
                ...
                if fy.date_from > self.date_to:
                    if (
                        "next_fy" not in params
                        or fy.date_from < params["next_fy"]["date_from"]
                    ):
                        params["next_fy"] = {
                            "date_from": fy.date_from,
                            "date_to": fy.date_to,
        ...
        def check_for_accrual_profile():
            ...
            for prop in ACCRUAL_MESSAGES.keys():
                if profile_account[prop].id:
                    if profile_account[prop].nature != ACCRUAL_MESSAGES[prop]["nature"]:
                        to_raise = True
                        raise_messages.append(ACCRUAL_MESSAGES[prop]["wrong_type"])
                else:
                    to_raise = True
                    raise_messages.append(ACCRUAL_MESSAGES[prop]["empty"])
            if to_raise:
                msg = "\n- ".join(raise_messages)
                raise Warning(f"Attenzione: Anomalie profilo contabile\n - {msg}")
        ...
        self.balance_model.browse(active_id).write({"last_update": datetime.today()})
