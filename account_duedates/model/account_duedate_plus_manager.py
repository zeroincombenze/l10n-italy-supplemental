...
class DueDateManager(models.Model):
    ...
    def _validate_duedates_date(self):
        ...
        if len(duedates_list) <= 1:
            ...
            return {
                ...
                "message": (
                    "Solo la prima scadenza può essere precedente " "alla data fattura"
                ),
            }
    ...
    def _validate_duedates_amount(self):
        ...
        duedates_amounts = [
            round(duedate.due_amount, precision) for duedate in self.duedate_line_ids
        ]
        ...
        if duedates_amounts and (difference != 0):  # Validation FAILED
            return {
                ...
                "message": (
                    "Il totale degli importi delle scadenze ({}) "
                    "deve coincidere con il totale della "
                    "registrazione ({}). Differenza: ({})"
                ).format(amounts_sum, amount_total, difference),
            }
    ...
    def _duedates_common(self, param_cm):
        ...
        if not param_cm["payment_terms"]:
            ...
            if param_cm["total_amount"] > 0:
                ...
                extra_line = self._extra_line(inv_date, payment_method_tax.id, types)
            ...
            # canonical lines
            new_dudate_lines = self._compute_duedates_lines(due_dates, param_cm, tax)
    ...
    def _compute_duedates_lines(self, due_dates, param_cm, tax):
        ...
        for due_date in due_dates:
            ...
            if not payment_method:
                ...
                error_msg = (
                    f"Termine di pagamento {pt.display_name} (id: {pt.id})"
    ...
    def _extra_duedate_line(self, param_cm, types_amount, payment_method_id):
        ...
        if bool(types_amount["ra_amount"]):
            if self.invoice_id.date:
                ra_date_fifteen = self.invoice_id.date_due + datetime.timedelta(days=15)
    ...
    def _compute_tax_to_add(self, payment_terms, types):
        ...
        if types["is_ra"]:
            tax_amount = (
                self.invoice_id.amount_tax - self.invoice_id.withholding_tax_amount
            )
    ...
    def _get_tax_type(self):
        return {
            ...
            "is_ra": bool(getattr(self.invoice_id, "withholding_tax_amount", None)),
