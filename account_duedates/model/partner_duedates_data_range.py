...
class PartnerDuedatesDatarange(models.Model):
    ...
    period_id = fields.Many2one(
        string="Periodo", comodel_name="date.range", required=True, domain=_get_period
    )

    split_date = fields.Date(string="Data di slittamento", required=True, default=False)
    ...
    def create(self, values):
        if values["split_date"]:
            ...
            if comparison_date <= data_range.date_end:
                raise UserError(
                    "La data di slittamento deve essere "
