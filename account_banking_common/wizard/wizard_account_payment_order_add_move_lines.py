...
class AccountPaymentAddMoveLines(models.TransientModel):
    ...
    def _payment_orders(self):
        ...
        if active_ids and len(active_ids) > 0:
            ...
            if lines[0].move_id.company_bank_id and lines[0].move_id.company_bank_id.id:
                ...
                for order in orders:
                    ...
                    for line in order.payment_line_ids:
                        if line.move_line_id.move_id.company_bank_id.id:
                            to_match_id = line.move_line_id.move_id.company_bank_id.id
    ...
    payment_orders = fields.Selection(
        string="Ordini di pagamento", required=True, selection="_payment_orders"
    )
    ...
    def _raise_on_errors(self, lines, payment_order):
        ...
        check_order_payment_method = defaultdict(lambda: {"count": 0, "name": None})
        ...
        for line in lines:
            ...
            if (
                ...
                and line.payment_method.id != order_payment_method_id
            ):
                check_order_payment_method[line.payment_method.id][
                    "name"
        ...
        if error_busy or error_method or error_order_payment_method:
            ...
            if error_order_payment_method:
                error_msg_method_order = self._error_msg_method_order(
                    check_order_payment_method, order_payment_method_name
                )
            # end if
    ...
    def _error_msg_method_order(check_order_payment_method, order_payment_method_name):
        ...
        msg += "\n - ".join(
            map(lambda x: x["name"], check_order_payment_method.values())
        )
