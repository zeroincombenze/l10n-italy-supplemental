...
class AccountPaymentGenerate(models.TransientModel):
    ...
    def _set_domain_journal(self):
        active_ids = self._context.get("active_ids")
        payment_method = False
        default_domain = [("type", "=", "bank"), ("bank_account_id", "!=", False)]
        if active_ids and len(active_ids) > 0:
            lines = self.env["account.move.line"].browse(active_ids)
            for line in lines:
                payment_method = line.payment_method
        ...
        if payment_method and payment_method.code:
            if payment_method.code == "invoice_financing":
                ...
                if default_mode_id:
                    journal_ids = self._set_journals_invoice_financing(default_mode_id)
    ...
    def on_change_journal_id(self):
        ...
        if active_ids and len(active_ids) > 0:
            ...
            if (
                ...
                return {"domain": {"journal_id": [("id", "in", tuple(journal_ids))]}}
    ...
    def on_change_payment_mode(self):
        ...
        if active_ids and len(active_ids) > 0:
            ...
            return {"domain": {"payment_mode_id": [("id", "in", tuple(mode_ids))]}}
        ...
        if len(active_ids) > 0:
            ...
            if self.payment_mode_id.payment_method_code == "invoice_financing":
                ...
                if self.journal_id and self.journal_id.id:
                    ...
                    elif journal.invoice_financing_evaluate not in [
                        "invoice_amount",
                        "taxable_amount",
                    ]:
                        raise UserError(
            ...
            if (
                self.journal_id.is_wallet
                and self.journal_id.bank_account_id.bank_is_wallet
            ):
                lines.write({"company_bank_id": self.journal_id.bank_account_id.id})
            ...
            lines.write({"payment_order": payment_order.id})

            if self.payment_mode_id.payment_method_code == "invoice_financing":
                if (
                    hasattr(payment_order, "bank_invoice_financing_amount")
                    and not payment_order.bank_invoice_financing_amount
                ):
    ...
    def _raise_on_errors(self, lines):
        ...
        for line in lines:
            ...
            if line.payment_method:
                ...
                date_tz = format_date(
                    self.env, fields.Date.context_today(self, date_format)
                )
                raise UserError(
                    "ATTENZIONE!\nMetodo di pagamento non "
                    "impostato nella scadenza {sk} del {dt}.".format(
                        sk=scadenza, dt=date_tz
                    )
                )
    ...
    def _error_msg_busy(busy_lines):
        msg = "ATTENZIONE!\nLe seguenti righe " "sono già parte di una distinta:\n\n - "
    ...
    def _error_msg_method(payment_methods):
        ...
        msg += "\n - ".join(map(lambda x: x["name"], payment_methods.values()))
    ...
    def _check_invoice_financing_line_bank(self, lines):
        ...
        if not account_invoice_bank:
            raise UserError("ATTENZIONE!\nConto bancario aziendale " "non impostato. ")
        # end if
    ...
    def _check_riba_supplier(lines):
        ...
        if reconciled_lines:
            msg = (
                "ATTENZIONE!\nLe seguenti righe " "sono già state riconciliate:\n\n - "
            )
    ...
    def _set_journals_invoice_financing(self, default_mode_id):
        ...
        if default_mode.bank_account_link == "fixed":
            if default_mode.fixed_journal_id.wallet_ids:
                children = [ch.id for ch in default_mode.fixed_journal_id.wallet_ids]
