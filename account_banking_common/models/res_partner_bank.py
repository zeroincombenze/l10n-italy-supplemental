...
class ResPartnerBank(models.Model):
    ...
    bank_is_wallet = fields.Boolean(
        string="Conto di portafoglio", default=_is_wallet_default
    )
    ...
    has_children = fields.Boolean(string="Padre", compute=_has_children)

    journal_is_wallet = fields.Boolean(
        string="Conto di portafoglio", related="journal_id.is_wallet"
    )

    journal_wallet_ids = fields.One2many(
        string="Conti di portafoglio", related="journal_id.wallet_ids"
    )

    journal_main_bank_account_id = fields.Many2one(
        string="Conto padre", related="journal_id.main_bank_account_id"
    )
    ...
    def _onchange_bank_is_wallet(self):
        ...
        if self.journal_id:
            _logger.info(
                "valore is_wallet res_partner_bank |{v}|".format(v=self.bank_is_wallet)
            )
            ...
            _logger.info(
                "valore is_wallet account_journal {id} |{v}|".format(
                    v=journal.is_wallet, id=journal.id
                )
            )
        else:
            _logger.info(
                "valore is_wallet account_journal {id} |{v}|".format(
                    v=self.journal_id.is_wallet, id=self.journal_id.id
                )
            )
        # end if
    ...
    def name_get(self):
        """
        Se il conto non ha banca (bank_id) e non è un conto di portafoglio
        (bank_is_wallet) eseguire solo super()
        altrimenti dopo
        Inizializzare descrizione vuota
        Se esiste il collegamento alla banca (bank_id) prelevare il nome della banca
        (max 10 caratteri, se portafoglio. altrimenti 20 caratteri)
        seguito da carattere " ("
        Se il conto è conto di portafoglio (bank_is_wallet) leggere IBAN di portafoglio
        e prelevare primi 4 caratteri seguiti da carattere "*"
        e ultimi 3 caratteri seguiti da "/"
        Leggere IBAN corrente e prelevare primi 4 caratteri seguiti da carattere "*"
        e ultimi 3 caratteri
        ...
        for t_record in res:
            ...
            if (
                (not record.bank_id and record.bank_is_wallet is False)
                or record.acc_type != "iban"
                or not record.acc_number
            ):
                ...
                chars_bank_name = (
                    16 if (record.bank_id and record.bank_is_wallet) else 31
                )
                ...
                if (
                    record.bank_is_wallet
                    and record.bank_main_bank_account_id.acc_number
                ):
                    siz = len(record.bank_main_bank_account_id.acc_number) - 3
                    # father account
                    disp_name += record.bank_main_bank_account_id.acc_number[0:5]
                    disp_name += "*"
                    disp_name += record.bank_main_bank_account_id.acc_number[siz:]
                    disp_name += "/"
                # end if
