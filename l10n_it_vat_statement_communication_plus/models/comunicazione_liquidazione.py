...
class ComunicazioneLiquidazione(models.Model):
    ...
    def _validate(self):
        ...
        if not self.taxpayer_fiscalcode \
            ...
            raise ValidationError(_("Taxpayer Fiscalcode is required. It's "
                                    "accepted codes with lenght 11 or 16 chars"
                                    )
                                  )
        ...
        if self.taxpayer_fiscalcode and len(self.taxpayer_fiscalcode) == 11\
            ...
            raise ValidationError(_("Declarant Fiscalcode is required. You can "
                                    "enable the section with different "
                                    "declarant option"))
...
class ComunicazioneLiquidazioneVp(models.Model):
    ...
    def _compute_imponibile_operazioni_attive(self, liq, period):
        ...
        for debit in liq.debit_vat_account_line_ids:
            if debit.tax_id.vsc_exclude_operation:
                continue
            self.imponibile_operazioni_attive += debit.base_amount

    def _compute_imponibile_operazioni_passive(self, liq, period):
        self.ensure_one()
        for credit in liq.credit_vat_account_line_ids:
            if credit.tax_id.vsc_exclude_operation:
                continue

            self.imponibile_operazioni_passive += credit.base_amount
