...
class res_country(orm.Model):
    ...
    _columns = {
        'codice_stato_agenzia_entrate':
            fields.char('Codice stato Agenzia Entrate', size=3),
        'inue':
            fields.boolean('Member of EU',
                           help='This country'
                                ' is member of the European Union.'),
        'blacklist':
            fields.boolean('Blacklist',
...
class account_journal(orm.Model):
    ...
    _columns = {
        ...
        'spesometro_operazione':
            fields.selection((('FA', 'Operazioni documentate da fattura'),
                              ('SA', 'Operazioni senza fattura'),
                              ('BL1',
                               'Operazioni con paesi'
                               ' con fiscalità privilegiata'),
                              ('BL2',
...
class res_partner(orm.Model):
    ...
    _columns = {
        ...
        'spesometro_leasing': fields.selection((('A', 'Autovettura'),
                                                ...
                                                ('E', 'Aeromobili')),
                                               'Tipo Leasing'),
        'spesometro_tipo_servizio':
            fields.selection((('cessione', 'Cessione Beni'),
                              ('servizi', 'Prestazione di servizi')),
                             'Tipo servizio',
                             help="Specificare per 'Operazioni con paesi"
                                  " con fiscalità privilegiata' "),
    }
...
class spesometro_configurazione(orm.Model):
    ...
    def _check_one_year(self, cr, uid, ids, context=None):
        for element in self.browse(cr, uid, ids, context=context):
            element_ids = self.search(
                cr, uid, [('anno', '=', element.anno)], context=context)
    ...
    _columns = {
        ...
        'quadro_fa_limite_importo':
            fields.float('Quadro FA - Limite importo'),
        'quadro_fa_limite_importo_line':
            fields.float('Quadro FA - Limite importo singola operaz.'),
        'quadro_sa_limite_importo':
            fields.float('Quadro SA - Limite importo'),
        'quadro_sa_limite_importo_line':
            fields.float('Quadro SA - Limite importo singola operaz.'),
        'quadro_bl_limite_importo':
            fields.float('Quadro BL - Limite importo'),
        'quadro_bl_limite_importo_line':
            fields.float('Quadro BL - Limite importo singola operaz.'),
        'quadro_se_limite_importo_line':
            fields.float('Quadro SE - Limite importo singola operaz.'),
    }
    _constraints = [
        (_check_one_year,
         'Error! Config for this year already exists.', ['anno']),
    ]
...
class spesometro_comunicazione(orm.Model):
    ...
    def _tot_operation_number(self, cr, uid, ids, field_names, args,
                              context=None):
        res = {}
    ...
    _columns = {
        ...
        'periodo':
            fields.selection((('anno', 'Annuale'),
                              ('trimestre', 'Trimestrale'),
                              ('mese', 'Mensile')),
                             'Periodo',
                             required=True),
        'anno':
            fields.integer('Anno', size=4, required=True),
        'trimestre': fields.integer('Trimestre', size=1),
        'mese':
            fields.selection((('1', 'Gennaio'),
                              ('2', 'Febbraio'),
                              ('3', 'Marzo'),
                              ('4', 'Aprile'),
                              ('5', 'Maggio'),
                             ...
                             'Mese'),
        'tipo':
            fields.selection((('ordinaria', 'Ordinaria'),
                              ('sostitutiva', 'Sostitutiva'),
                              ('annullamento', 'Annullamento')),
                             'Tipo comunicazione',
                             required=True),
        'comunicazione_da_sostituire_annullare':
            fields.integer('Protocollo comunicaz. da sostituire/annullare'),
        'documento_da_sostituire_annullare':
            fields.integer('Protocollo documento da sostituire/annullare'),
        'formato_dati': fields.selection((('aggregati', 'Dati Aggregati'),
                                          ('analitici', 'Dati Analitici')),
                                         'Formato dati',
                                         readonly=True),
        'codice_fornitura':
            fields.char('Codice fornitura',
                        readonly=True,
                        size=5,
                        help='Impostare a "NSP00" '),
        'tipo_fornitore':
            fields.selection((('01', 'Invio propria comunicazione'),
                              ('10', 'Intermediario')),
                             'Tipo fornitore'),
        'codice_fiscale_fornitore':
            fields.char('Codice fiscale Fornitore',
                        size=16,
                        help="Deve essere uguale al Codice fiscale "
                             "dell'intermediario (campo 52 del record B)"
                             " se presente, altrimenti al Codice fiscale"
                             " del soggetto tenuto alla comunicazione"
                             " (campo 41 del record B) se presente,"
                             " altrimenti al Codice fiscale del"
                             " soggetto obbligato (campo 2 del record B)"),
        #
        ...
        'numero_totale_invii':
            fields.integer('Numero totale invii telematici', readonly=True),
        #
        ...
        'soggetto_codice_attivita':
            fields.char('Codice attività',
                        size=6,
        ...
        'soggetto_forma_giuridica':
            fields.selection((('persona_giuridica', 'Persona Giuridica'),
                              ('persona_fisica', 'Persona Fisica')),
                             'Forma Giuridica'),
        'soggetto_pf_cognome': fields.char('Cognome', size=24, help=""),
        'soggetto_pf_nome': fields.char('Nome', size=20, help=""),
        'soggetto_pf_sesso': fields.selection((('M', 'M'),
                                               ('F', 'F')),
                                              'Sesso'),
        'soggetto_pf_data_nascita': fields.date('Data di nascita'),
        'soggetto_pf_comune_nascita':
            fields.char('Comune o stato estero di nascita', size=40),
        'soggetto_pf_provincia_nascita':
            fields.char('Provincia', size=2),
        'soggetto_pg_denominazione':
            fields.char('Denominazione', size=60),
        # Soggetto tenuto alla comunicazione
        'soggetto_cm_forma_giuridica':
            fields.selection((('persona_giuridica', 'Persona Giuridica'),
                              ('persona_fisica', 'Persona Fisica')),
                             'Forma Giuridica'),
        'soggetto_cm_codice_fiscale':
            fields.char('Codice Fiscale',
                        size=16,
                        help="Soggetto che effettua la comunicazione se "
                             "diverso dal soggetto tenuto alla comunicazione"),
        'soggetto_cm_pf_cognome': fields.char('Cognome', size=24, help=""),
        'soggetto_cm_pf_nome': fields.char('Nome', size=20, help=""),
        'soggetto_cm_pf_sesso': fields.selection((('M', 'M'),
                                                  ('F', 'F')),
                                                 'Sesso'),
        'soggetto_cm_pf_data_nascita': fields.date('Data di nascita'),
        'soggetto_cm_pf_comune_nascita':
            fields.char('Comune o stato estero di nascita',
                        size=40),
        'soggetto_cm_pf_provincia_nascita': fields.char('Provincia', size=2),
        'soggetto_cm_pf_codice_carica':
            fields.integer('Codice Fiscale', size=2, help=""),
        'soggetto_cm_pf_data_inizio_procedura':
            fields.date('Data inizio procedura'),
        'soggetto_cm_pf_data_fine_procedura':
            fields.date('Data fine procedura'),
        'soggetto_cm_pg_denominazione':
            fields.char('Denominazione', size=60),
        # Soggetto incaricato alla trasmissione
        'soggetto_trasmissione_codice_fiscale':
            fields.char('Codice Fiscale',
                        size=16,
                        help="Intermediario che effettua la trasmissione"
                             " telematica"),
        'soggetto_trasmissione_numero_CAF':
            fields.integer('Nr iscrizione albo del C.A.F.',
                           size=5,
                           help="Intermediario che effettua la trasmissione"
                                " telematica"),
        'soggetto_trasmissione_impegno':
            fields.selection((('1', 'Soggetto obbligato'),
                              ('2', 'Intermediario')),
                             'Impegno trasmissione'),
        'soggetto_trasmissione_data_impegno':
            fields.date('Data data impegno'),
        'line_FA_ids':
            fields.one2many('spesometro.comunicazione.line.fa',
                            'comunicazione_id', 'Quadri FA'),
        'line_SA_ids':
            fields.one2many('spesometro.comunicazione.line.sa',
                            'comunicazione_id', 'Quadri SA'),
        'line_BL_ids':
            fields.one2many('spesometro.comunicazione.line.bl',
                            'comunicazione_id', 'Quadri BL'),
        'line_FE_ids':
            fields.one2many('spesometro.comunicazione.line.fe',
                            'comunicazione_id', 'Quadri FE'),
        'line_FR_ids':
            fields.one2many('spesometro.comunicazione.line.fr',
                            'comunicazione_id', 'Quadri FR'),
        'line_NE_ids':
            fields.one2many('spesometro.comunicazione.line.ne',
                            'comunicazione_id', 'Quadri NE'),
        'line_NR_ids':
            fields.one2many('spesometro.comunicazione.line.nr',
                            'comunicazione_id', 'Quadri NR'),
        'line_DF_ids':
            fields.one2many('spesometro.comunicazione.line.df',
                            'comunicazione_id', 'Quadri DF'),
        'line_FN_ids':
            fields.one2many('spesometro.comunicazione.line.fn',
                            'comunicazione_id', 'Quadri FN'),
        'line_SE_ids':
            fields.one2many('spesometro.comunicazione.line.se',
                            'comunicazione_id', 'Quadri SE'),
        'line_TU_ids':
            fields.one2many('spesometro.comunicazione.line.tu',
                            'comunicazione_id', 'Quadri TU'),
        'totale_FA':
            fields.function(_tot_operation_number,
                            string='Tot operazioni FA',
                            type='integer',
                            multi='operation_number'),
        'totale_SA':
            fields.function(_tot_operation_number,
                            string='Tot operazioni SA',
                            type='integer',
                            multi='operation_number'),
        'totale_BL1':
            fields.function(_tot_operation_number,
                            string='Tot operazioni BL - '
        ...
        'totale_BL3':
            fields.function(_tot_operation_number,
                            string='Tot operazioni BL - Acquisti servizi'
                                   ' non soggetti non residenti',
                            type='integer',
                            multi='operation_number'),
        'totale_FE':
            fields.function(_tot_operation_number,
                            string='Tot operazioni FE',
                            type='integer',
                            multi='operation_number'),
        'totale_FE_R':
            fields.function(_tot_operation_number,
                            string='Tot operazioni FE doc riepil.',
                            type='integer',
                            multi='operation_number'),
        'totale_FR':
            fields.function(_tot_operation_number,
                            string='Tot operazioni FR',
                            type='integer',
                            multi='operation_number'),
        'totale_FR_R':
            fields.function(_tot_operation_number,
                            string='Tot operazioni FR doc riepil.',
                            type='integer',
                            multi='operation_number'),
        'totale_NE':
            fields.function(_tot_operation_number,
                            string='Tot operazioni NE',
                            type='integer',
                            multi='operation_number'),
        'totale_NR':
            fields.function(_tot_operation_number,
                            string='Tot operazioni NR',
                            type='integer',
                            multi='operation_number'),
        'totale_DF':
            fields.function(_tot_operation_number,
                            string='Tot operazioni DF',
                            type='integer',
                            multi='operation_number'),
        'totale_FN':
            fields.function(_tot_operation_number,
                            string='Tot operazioni FN',
                            type='integer',
                            multi='operation_number'),
        'totale_SE':
            fields.function(_tot_operation_number,
                            string='Tot operazioni SE',
                            type='integer',
                            multi='operation_number'),
        'totale_TU':
            fields.function(_tot_operation_number,
                            string='Tot operazioni TU',
                            type='integer',
                            multi='operation_number'),
    }
    ...
    def partner_is_from_san_marino(self, cr, uid, move, invoice, arg):
        # configurazione
        anno_competenza = datetime.datetime.strptime(
            move.period_id.date_start, "%Y-%m-%d").year
        configurazione_ids = self.pool.get('spesometro.configurazione').\
            search(cr, uid, [('anno', '=', anno_competenza)])
        if not configurazione_ids:
            raise orm.except_orm(_('Configurazione mancante!'), _(
                "Configurare l'anno relativo alla comunicazione"))
        configurazione = self.pool.get('spesometro.configurazione').browse(
    ...
    def compute_invoice_amounts(self, cr, uid, move, invoice, arg):
        '''
        Calcolo totali documento. Dall'imponibile
        vanno esclusi gli importi esclusi, fuori campo o esenti
        '''
        res = {
            'amount_untaxed': 0,
            'amount_tax': 0,
    ...
    def truncate_values(self, cr, uid, ids, context=None):
        for com in self.browse(cr, uid, ids):
            ...
            for line in com.line_BL_ids:
                ...
                self.pool.get('spesometro.comunicazione.line.bl').write(
                    cr, uid, [line.id], vals)
    ...
    def validate_lines(self, cr, uid, ids, context=None):
        for com in self.browse(cr, uid, ids):
            # configurazione
            configurazione_ids = self.pool.get('spesometro.configurazione').\
                search(cr, uid, [('anno', '=', com.anno)])
            if not configurazione_ids:
                raise orm.except_orm(
                    _('Configurazione mancante!'),
                    _("Configurare l'anno relativo alla comunicazione"))
            configurazione = self.pool.get('spesometro.configurazione').browse(
                cr, uid, configurazione_ids[0])
            for line in com.line_FA_ids:
            ...
            for line in com.line_BL_ids:
                if configurazione.quadro_bl_limite_importo:
                    ...
                    if importo_test and \
                            importo_test < \
                            configurazione.quadro_bl_limite_importo:
    ...
    def validate_operation(self, cr, uid, move, invoice, arg):
        ...
        configurazione = self.pool.get('spesometro.configurazione').browse(
            cr, uid, configurazione_ids[0])
        doc_vals = self.pool.get('spesometro.comunicazione').\
            compute_invoice_amounts(cr, uid, move, invoice, arg)
        ...
        if arg['quadro'] == 'FA':
            if configurazione.quadro_fa_limite_importo_line:
                if not doc_vals.get('amount_untaxed', 0) or \
                        doc_vals.get('amount_untaxed', 0) < \
                        configurazione.quadro_fa_limite_importo_line:
                    return False
        if arg['quadro'] == 'SA':
            if configurazione.quadro_sa_limite_importo_line:
                if not doc_vals.get('amount_total', 0) or \
                        doc_vals.get('amount_total', 0) < \
        ...
        if arg['quadro'] == 'BL':
            if configurazione.quadro_bl_limite_importo_line:
                if not doc_vals.get('amount_total', 0) or \
                        doc_vals.get('amount_total', 0) < \
                        configurazione.quadro_bl_limite_importo_line:
                    return False
        if arg['quadro'] == 'SE':
        ...
        if arg['formato_dati'] == 'aggregati' and \
                self.partner_is_from_san_marino(cr, uid, move, invoice, arg):
            return False
        return True
    ...
    def get_define_quadro(self, cr, uid, move, invoice, arg):
        ...
        if arg['formato_dati'] == 'analitici':
            ...
            if operazione == 'FA' or operazione == 'DR':
                ...
                quadro = 'FN'
            # Operazioni con paesi con fiscalità privilegiata - Acquisti di
            # servizi da soggetti non residenti
    ...
    def genera_comunicazione(self, cr, uid, params, context=None):
        _logger.debug('params=%s, ctx=%s', str(params), str(context))
        ...
        def _get_periods(cr, uid, params, context=None):
            ...
            if params.get('periodo') == 'mese':
                period_date_start = datetime.date(
                    params.get('anno'), int(params.get('mese')), 1)
                sql_where += " AND p.date_start = date(%(period_date_start)s) "
                search_params.update({
                    'period_date_start': period_date_start,
                })
            ...
            if params.get('periodo') == 'trimestre':
                if params.get('trimestre') == 1:
                    period_date_start = datetime.date(params.get('anno'), 1, 1)
                    period_date_start = datetime.date(
                        params.get('anno'), 3, 31)
                elif params.get('trimestre') == 2:
                    period_date_start = datetime.date(params.get('anno'), 3, 1)
                    period_date_start = datetime.date(
                        params.get('anno'), 6, 30)
                elif params.get('trimestre') == 2:
                    period_date_start = datetime.date(params.get('anno'), 7, 1)
                    period_date_start = datetime.date(
                        params.get('anno'), 9, 30)
                elif params.get('trimestre') == 2:
                    period_date_start = datetime.date(
                        params.get('anno'), 10, 1)
                    period_date_start = datetime.date(
                        params.get('anno'), 12, 31)
                else:
                    raise orm.except_orm(_('Dato errato!'), _(
                        "Errore nel valore del trimestre"))
                sql_where += " AND p.date_start >= date(%(period_date_start)s)"
                sql_where += " AND p.date_stop <=date(%(period_date_stop)s) "
                search_params.update({
                    'period_date_start': period_date_start,
        ...
        def _genera_testata(cr, uid, params, context=None):
            ...
            company = self.pool.get('res.company').browse(
                cr, uid, params['company_id'])
            # progressivo telematico :" il progressivo deve essere univoco e
            # crescente (con incrementi di una unità per ogni file prodotto)"
            if params['tipo'] == 'ordinaria':
                com_search = [('tipo', '=', 'ordinaria')]
                com_last_ids = self.search(cr,
                                           uid,
                                           com_search,
                ...
                com_next_prg = 1
                if com_last_ids:
                    com_next_prg = self.browse(cr, uid, com_last_ids[
                                               0]).progressivo_telematico + 1
            progressivo_telematico = com_next_prg
            # vat
            if company.partner_id.vat:
                partita_iva = company.partner_id.vat[2:]
            else:
                partita_iva = '{0:11s}'.format("".zfill(11))
            # codice fiscale soggetto incaricato alla trasmissione
            codice_fiscale_incaricato_trasmissione = ''
            if params.get('tipo_fornitore') == '10' and \
                    params.get('partner_intermediario', False):
                partner_intermediario = self.pool.get('res.partner').browse(
                    cr, uid, params.get('partner_intermediario'))
                codice_fiscale_incaricato_trasmissione = \
                    partner_intermediario.fiscalcode or False
            ...
            # Considerazione: se se lunghezza codice fiscale < 16 allora è la
            # P.Iva e quindi trattasi di soggetto giuridico
            tipo_persona = 'persona_fisica'
            if company.partner_id.fiscalcode and \
                    len(company.partner_id.fiscalcode) < 16:
            ...
            values = {
                ...
                'soggetto_codice_fiscale':
                company.partner_id and company.partner_id.fiscalcode or '',
                'soggetto_partitaIVA': partita_iva,
                'soggetto_telefono':
                company.partner_id and company.partner_id.phone or '',
                'soggetto_fax':
                company.partner_id and company.partner_id.fax or '',
                'soggetto_email':
                company.partner_id and company.partner_id.email or '',
                'soggetto_forma_giuridica': tipo_persona,
                'soggetto_pg_denominazione':
                company.partner_id and company.partner_id.name or
                company.name or '',
        ...
        configurazione_ids = self.pool.get('spesometro.configurazione').search(
            cr, uid, [('anno', '=', params.get('anno'))])
        if not configurazione_ids:
            raise orm.except_orm(_('Configurazione mancante!'), _(
                "Configurare l'anno relativo alla comunicazione"))
        ...
        _logger.debug('period_ids %s', str(period_ids))

        # journal
        journal_search = [('spesometro', '=', True)]
        journal_ids = journal_obj.search(
            cr, uid, journal_search, context=context)
        _logger.debug('journal_ids %s', str(journal_ids))

        # Partners to exclude
        partner_search = [('spesometro_escludi', '=', True)]
        partner_to_exclude_ids = partner_obj.search(
            cr, uid, partner_search, context=context)
        _logger.debug('partner_to_exclude_ids %s', str(partner_to_exclude_ids))
        ...
        move_ids = account_move_obj.search(
            cr, uid, move_search, context=context)
        _logger.debug('move_ids %s', str(move_ids))
        ...
        for move in self.pool.get('account.move').browse(cr, uid, move_ids):
            ...
            invoice_ids = invoice_obj.search(
                cr, uid, invoice_search, context=context)
            _logger.debug('invoice_ids %s', str(invoice_ids))
            if not invoice_ids:
                continue
...
class spesometro_comunicazione_line_FA(orm.Model):
    ...
    _columns = {
        'comunicazione_id':
            fields.many2one('spesometro.comunicazione',
                            'Comunicazione',
                            ondelete='cascade'),
        ...
        'noleggio':
            fields.selection((('A', 'Autovettura'),
                              ('B', 'Caravan'),
                              ('C', 'Altri Veicoli'),
                              ('D', 'Unità  da diporto'),
                              ('E', 'Aeromobii')),
                             'Leasing'),
        'numero_operazioni_attive_aggregate':
            fields.integer('Nr op. attive', size=16),
        'numero_operazioni_passive_aggregate':
            fields.integer('Nr op. passive', size=16),
        'attive_imponibile_non_esente':
            fields.float('Tot impon., non impon ed esenti',
                         digits_compute=dp.get_precision('Account'),
                         help="Totale operazioni imponibili, non imponibili "
                              "ed esenti"),
        'attive_imposta':
            fields.float(' Tot imposta',
                         digits_compute=dp.get_precision('Account'),
                         help="Totale imposta"),
        'attive_operazioni_iva_non_esposta':
            fields.float('Totale operaz. IVA non esposta',
                         digits_compute=dp.get_precision('Account'),
                         help="Totale operazioni con IVA non esposta"),
        'attive_note_variazione':
            fields.float('Totale note variazione',
                         digits_compute=dp.get_precision('Account'),
                         help="Totale note di variazione a debito per "
                              "la controparte"),
        'attive_note_variazione_imposta':
            fields.float('Totale imposta note variazione',
                         digits_compute=dp.get_precision('Account'),
                         help="Totale imposta sulle note di variazione "
                              "a debito"),
        'passive_imponibile_non_esente':
            fields.float('Tot impon., non impon ed esenti',
                         digits_compute=dp.get_precision('Account'),
        ...
        'passive_note_variazione':
            fields.float('Totale note variazione',
                         digits_compute=dp.get_precision('Account'),
                         help="Totale note di variazione a credito per "
                              "la controparte"),
        'passive_note_variazione_imposta':
            fields.float('Totale imposta note variazione',
                         digits_compute=dp.get_precision('Account'),
    ...
    def add_line(self, cr, uid, move, invoice, arg):
        ...
        com_line_search = [('comunicazione_id', '=', comunicazione_id),
        ...
        if not com_line_ids:
            ...
            if arg.get('segno', False) == 'attiva':
                ...
                if 'refund' in move.journal_id.type:
                    val['attive_note_variazione'] = doc_vals.get(
                        'amount_untaxed', 0)
                    val['attive_note_variazione_imposta'] = doc_vals.get(
                        'amount_tax', 0)
                else:
                    if arg.get('operazione_iva_non_esposta', False):
                        ...
                        val['attive_imponibile_non_esente'] = doc_vals.get(
                            'amount_untaxed', 0)
            ...
            # passive
                ...
                if 'refund' in move.journal_id.type:
                    val['passive_note_variazione'] = doc_vals.get(
                        'amount_untaxed', 0)
                    val['passive_note_variazione_imposta'] = doc_vals.get(
                        'amount_tax', 0)
                else:
                    if arg.get('operazione_iva_non_esposta', False):
                        ...
                        val['passive_imponibile_non_esente'] = doc_vals.get(
                            'amount_untaxed', 0)
            ...
            for com_line in self.browse(cr, uid, com_line_ids):
                ...
                if arg.get('segno', False) == 'attiva':
                    val['numero_operazioni_attive_aggregate'] = \
                        com_line.numero_operazioni_attive_aggregate + 1
                    if 'refund' in move.journal_id.type:
                        val['attive_note_variazione'] = \
                            com_line.attive_note_variazione + \
                            doc_vals.get('amount_untaxed', 0)
                        val['attive_note_variazione_imposta'] = \
                            com_line.attive_note_variazione_imposta + \
                            doc_vals.get('amount_tax', 0)
                    else:
                        if arg.get('operazione_iva_non_esposta', False):
                            val['attive_operazioni_iva_non_esposta'] = \
                            ...
                            val['attive_imponibile_non_esente'] = \
                                com_line.attive_imponibile_non_esente + \
                                doc_vals.get('amount_untaxed', 0)
                            val['attive_imposta'] = \
                                com_line.attive_imposta + \
                                doc_vals.get('amount_tax', 0)
                # passive
                else:
                    val['numero_operazioni_passive_aggregate'] = \
                        com_line.numero_operazioni_passive_aggregate + 1
                    if 'refund' in move.journal_id.type:
                        val['passive_note_variazione'] = \
                            com_line.passive_note_variazione + \
                            doc_vals.get('amount_untaxed', 0)
                        val['passive_note_variazione_imposta'] = \
                            com_line.passive_note_variazione_imposta + \
                            doc_vals.get('amount_tax', 0)
                    else:
                        if arg.get('operazione_iva_non_esposta', False):
                            val['passive_operazioni_iva_non_esposta'] = \
                            ...
                            val['passive_imponibile_non_esente'] = \
                                com_line.passive_imponibile_non_esente + \
                                doc_vals.get('amount_untaxed', 0)
                            val['passive_imposta'] = \
                                com_line.passive_imposta + \
                                doc_vals.get('amount_tax', 0)
...
class spesometro_comunicazione_line_SA(orm.Model):
    ...
    _columns = {
        ...
        'codice_fiscale': fields.char('Codice Fiscale', size=16),
        'numero_operazioni':
            fields.integer('Numero operazioni'),
        'importo_complessivo':
            fields.float('Importo complessivo',
                         digits_compute=dp.get_precision('Account')),
        'noleggio':
            fields.selection((('A', 'Autovettura'),
                              ('B', 'Caravan'),
    ...
    def add_line(self, cr, uid, move, invoice, arg):
        ...
        if com_line_ids:
            for com_line in self.browse(cr, uid, com_line_ids):
                ...
                val['importo_complessivo'] = com_line.importo_complessivo + \
                    doc_vals.get('amount_total', 0)
...
class spesometro_comunicazione_line_BL(orm.Model):
    ...
    - Operazioni con soggetti non residenti (è obbligatorio
      compilare le sezioni BL001, BL002 e almeno un campo delle sezioni
      BL003 e BL006)
    ...
    _columns = {
        ...
        'importo_complessivo':
            fields.integer('Importo complessivo',
                           digits_compute=dp.get_precision('Account')),
        'noleggio':
            fields.selection((('A', 'Autovettura'),
                              ('B', 'Caravan'),
        ...
        'pf_comune_stato_nascita':
            fields.char('Comune o stato estero di nascita', size=40),
        'pf_provincia_nascita': fields.char('Provincia', size=2),
        'pf_codice_stato_estero':
            fields.char('Codice Stato Estero',
                        size=3,
                        help="Deve essere uno di quelli presenti nella "
                             "tabella 'elenco dei paesi e territori esteri' "
                             "pubblicata nelle istruzioni del modello Unico"),
        'pg_denominazione':
            fields.char('Denominazione/Ragione sociale', size=60),
        'pg_citta_estera_sede_legale':
            fields.char('Città estera delle Sede legale', size=40),
        'pg_codice_stato_estero':
            fields.char('Codice Stato Estero',
                        ...
                        help="Deve essere uno di quelli presenti nella "
                             "tabella 'elenco dei paesi e territori esteri' "
                             "pubblicata nelle istruzioni del modello Unico"),
        'pg_indirizzo_sede_legale':
            fields.char('Indirizzo sede legale', size=60),
        'codice_identificativo_IVA':
            fields.char('Codice identificativo IVA', size=16),
        'operazione_fiscalita_privilegiata':
            fields.boolean('Operazione con pesei con fiscalità privilegiata'),
        'operazione_con_soggetti_non_residenti':
            fields.boolean('Operazione con soggetto non residente'),
        'Acquisto_servizi_da_soggetti_non_residenti':
            fields.boolean('Acquisto di servizi da soggetti non residenti'),
        'attive_importo_complessivo':
            fields.float('Tot operaz. attive impon., non impon ed esenti',
                         digits_compute=dp.get_precision('Account'),
                         help="Totale operazioni imponibili, non imponibili "
                              "ed esenti"),
        'attive_imposta':
            fields.float('Tot operaz. attive imposta',
                         digits_compute=dp.get_precision('Account'),
                         help="Totale imposta"),
        'attive_non_sogg_cessione_beni':
            fields.float('Operaz.attive non soggette ad IVA - Cessione beni',
                         digits_compute=dp.get_precision('Account'),
                         help="Totale operazioni imponibili, non imponibili "
                              "ed esenti"),
        'attive_non_sogg_servizi':
            fields.float('Operaz.attive non soggette ad IVA - Servizi',
                         digits_compute=dp.get_precision('Account'),
                         help="Totale operazioni imponibili, non imponibili "
                              "ed esenti"),
        'attive_note_variazione':
            fields.float('Totale note variazione',
                         digits_compute=dp.get_precision('Account'),
                         help="Totale note di variazione a debito per la "
                              "controparte"),
        'attive_note_variazione_imposta':
            fields.float('Totale imposta note variazione',
                         digits_compute=dp.get_precision('Account'),
                         help="Totale imposta sulle note di variazione"
                              " a debito"),
        'passive_importo_complessivo':
            fields.float('Tot operaz. passive impon., non impon ed esenti',
                         digits_compute=dp.get_precision('Account'),
                         help="Totale operazioni imponibili, non imponibili "
                              "ed esenti"),
        'passive_imposta':
            fields.float('Tot operaz. passive imposta',
                         digits_compute=dp.get_precision('Account'),
                         help="Totale imposta"),
        'passive_non_sogg_importo_complessivo':
            fields.float('Operaz.passive non soggette ad IVA',
                         digits_compute=dp.get_precision('Account'),
                         help="Totale operazioni imponibili, non imponibili "
                              "ed esenti"),
        'passive_note_variazione':
        ...
        'passive_note_variazione_imposta':
            fields.float('Totale imposta note variazione',
                         digits_compute=dp.get_precision('Account'),
                         help="Totale imposta sulle note di variazione a "
                              "debito"),
    }
    ...
    def add_line(self, cr, uid, move, invoice, arg):
        ...
        if not com_line_ids:
            val = {
                ...
                'pg_citta_estera_sede_legale':
                move.partner_id.address[0].city or False,
                'pg_codice_stato_estero':
                move.partner_id.address[0].country_id.
                codice_stato_agenzia_entrate or '',
                'pg_indirizzo_sede_legale':
                move.partner_id.address[0].street or False,
            ...
            # attive
            if arg.get('segno', False) == 'attiva':
                if val['operazione_fiscalita_privilegiata'] or \
                        val['operazione_con_soggetti_non_residenti']:
                    val['attive_importo_complessivo'] = doc_vals.get(
                        'amount_total', 0)
                    val['attive_imposta'] = doc_vals.get('amount_tax', 0)
                if val['operazione_fiscalita_privilegiata'] is True:
                    if move.partner_id.spesometro_operazione == 'cessioni':
                        val['attive_non_sogg_cessione_beni'] = doc_vals.get(
                            'amount_total', 0)
                    else:
                        val['attive_non_sogg_servizi'] = doc_vals.get(
                            'amount_total', 0)
                if 'refund' in move.journal_id.type:
                    val['attive_note_variazione'] = doc_vals.get(
                        'amount_untaxed', 0)
            ...
            # passive
                ...
                if val['operazione_fiscalita_privilegiata'] or \
                        val['operazione_con_soggetti_non_residenti'] or \
                        val['Acquisto_servizi_da_soggetti_non_residenti']:
                    val['passive_importo_complessivo'] = doc_vals.get(
                        'amount_total', 0)
                    val['passive_imposta'] = doc_vals.get('amount_tax', 0)
                if val['operazione_fiscalita_privilegiata'] is True:
                    val['passive_non_sogg_importo_complessivo'] = doc_vals.get(
                        'amount_total', 0)
                if 'refund' in move.journal_id.type:
                    val['passive_note_variazione'] = doc_vals.get(
                        'amount_untaxed', 0)
                ...
                if arg.get('segno', False) == 'attiva':
                    if val['operazione_fiscalita_privilegiata'] or \
                        ...
                        val['attive_importo_complessivo'] = \
                            com_line.attive_importo_complessivo + \
                            doc_vals.get('amount_total', 0)
                        val['attive_imposta'] = com_line.attive_imposta + \
                            doc_vals.get('amount_tax', 0)
                    if val['operazione_fiscalita_privilegiata'] is True:
                        if move.partner_id.spesometro_operazione == \
                                'cessioni':
                            ...
                            val['attive_non_sogg_servizi'] = \
                                com_line.attive_non_sogg_servizi + \
                                doc_vals.get('amount_total', 0)
                    if 'refund' in move.journal_id.type:
                        val['attive_note_variazione'] = \
                            com_line.attive_note_variazione + \
                ...
                # passive
                    ...
                    if val['operazione_fiscalita_privilegiata'] or \
                        ...
                        val['passive_importo_complessivo'] = \
                            com_line.passive_importo_complessivo + \
                            doc_vals.get('amount_total', 0)
                        val['passive_imposta'] = com_line.passive_imposta + \
                            doc_vals.get('amount_tax', 0)
                    if val['operazione_fiscalita_privilegiata'] is True:
...
class spesometro_comunicazione_line_FE(orm.Model):
    ...
    _columns = {
        ...
        'numero_fattura': fields.char('Numero Fattura - Doc riepilog.',
                                      size=16),
        'importo':
            fields.float('Importo',
                         digits_compute=dp.get_precision('Account')),
        'imposta':
            fields.float('Imposta',
                         digits_compute=dp.get_precision('Account')),
    }
...
class spesometro_comunicazione_line_FN(orm.Model):
    ...
    _columns = {
        ...
        'pf_cognome':
            fields.char('Cognome', size=24, help=""),
        'pf_nome':
            fields.char('Nome', size=20, help=""),
        'pf_data_nascita':
            fields.date('Data di nascita'),
        'pf_comune_stato_nascita':
            fields.char('Comune o stato estero di nascita', size=40),
        'pf_provincia_nascita':
            fields.char('Provincia', size=2),
        'pf_codice_stato_estero_domicilio':
            fields.char('Codice Stato Estero del Domicilio',
                        size=3,
                        help="Deve essere uno di quelli presenti nella "
                             "tabella 'elenco dei paesi e territori esteri' "
                             "pubblicata nelle istruzioni del modello Unico"),
        ...
        'pg_codice_stato_estero_domicilio':
            fields.char('Codice Stato Estero del Domicilio',
                        ...
                        help="Deve essere uno di quelli presenti nella "
                             ...
                             "pubblicata nelle istruzioni del modello Unico"),
        'pg_indirizzo_sede_legale':
            fields.char('Indirizzo legale', size=40),
        'data_emissione': fields.date('Data emissione'),
        'data_registrazione': fields.date('Data registrazione'),
        'numero_fattura':
            fields.char('Numero Fattura/Doc riepilog.', size=16),
        'noleggio':
            fields.selection((('A', 'Autovettura'),
                              ('B', 'Caravan'),
                              ('C', 'Altri Veicoli'),
...
class spesometro_comunicazione_line_SE(orm.Model):
    '''
    QUADRO SE - Acquisti di servizi da non residenti e
    ...
    _columns = {
        ...
        'pg_indirizzo_sede_legale':
            fields.char('Indirizzo legale', size=40),
        'codice_identificativo_IVA':
            fields.char('Codice Identificativo IVA (037=San Marino)',
                        size=3),
        'data_emissione': fields.date('Data emissione'),
        'data_registrazione':
            fields.date('Data registrazione'),
        'numero_fattura':
            fields.char('Numero Fattura/Doc riepilog.', size=16),
        'importo':
            fields.float('Importo/imponibile',
                         digits_compute=dp.get_precision('Account')),
        'imposta':
            fields.float('Imposta',
                         digits_compute=dp.get_precision('Account')),
    ...
    def add_line(self, cr, uid, move, invoice, arg):
        ...
        doc_vals = self.pool.get('spesometro.comunicazione').\
            compute_invoice_amounts(cr,
                                    ...
                                    arg)
        # Indirizzo
        address = self.pool.get('spesometro.comunicazione').\
            _get_partner_address_obj(cr,
                                     uid,
                                     move,
                                     invoice,
                                     arg)
        ...
        codice_identificativo_iva = ''
        if self.pool.get('spesometro.comunicazione').\
                partner_is_from_san_marino(cr, uid, move, invoice, arg):
...
class spesometro_comunicazione_line_TU(orm.Model):
    ...
    _columns = {
        ...
        'data_nascita':
            fields.date('Data di nascita'),
        'comune_stato_nascita':
            fields.char('Comune o stato estero di nascita', size=40),
        'provincia_nascita':
            fields.char('Provincia', size=2),
        'citta_estera_residenza':
            fields.char('Città Estera di residenza', size=40),
        'codice_stato_estero':
            fields.char('Codice Stato Estero',
                        size=3,
                        help="Deve essere uno di quelli presenti nella "
                             "tabella 'elenco dei paesi e territori esteri' "
                             "pubblicata nelle istruzioni del modello Unico"),
        'indirizzo_estero_residenza':
            fields.char('Indirizzo Estero di residenza', size=40),
        'data_emissione': fields.date('Data emissione'),
        'data_registrazione':
            fields.date('Data registrazione'),
        'numero_fattura':
