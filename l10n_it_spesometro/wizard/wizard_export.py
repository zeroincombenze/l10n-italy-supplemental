...
class wizard_spesometro_export(osv.osv_memory):
    ...
    def _record_A(self, cr, uid, comunicazione, context=None):
        ...
        #rcd += '{0:4s}'.format(str(comunicazione.progressivo_telematico).zfill(4)) # dich.su più invii: Progressivo dell'invio telematico 
        rcd += '{0:4s}'.format("0".zfill(4)) # dich.su più invii: Progressivo dell'invio telematico 
        rcd += '{0:4s}'.format("0".zfill(4)) # dich.su più invii: Numero totale degli invii telematici
    ...
    def _record_B(self, cr, uid, comunicazione, context=None):
        ...
        if not comunicazione.soggetto_codice_fiscale:
            raise osv.except_osv(_('Errore comunicazione!'),_("Manca il codice fiscale del soggetto obbligato alla trasmissione"))
        ...
        rcd += '{0:6s}'.format(comunicazione.soggetto_codice_attivita) # CODICE attività  (6 caratteri) --> obbligatorio
        tel = comunicazione.soggetto_telefono
        rcd += '{0:12s}'.format(tel.replace(' ','').replace('-','').replace('.','') or '') # telefono
        fax = comunicazione.soggetto_fax
        rcd += '{0:12s}'.format(fax.replace(' ','').replace('-','').replace('.','') or '') # fax
        ...
        rcd += '{0:4d}'.format(comunicazione.anno) # anno riferimento
        ...
        if comunicazione.soggetto_cm_codice_fiscale == '':
            ...
            rcd += '{0:40s}'.format("") # comune di nascita
            rcd += '{0:2s}'.format("") # provincia comune di nascita
            rcd += '{0:60s}'.format(comunicazione.soggetto_cm_pg_denominazione) # persona giuridica
